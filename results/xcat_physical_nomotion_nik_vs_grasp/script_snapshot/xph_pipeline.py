...
def build_train(dev):
    ...
    nz = KSpaceNormalizer(); nz.fit(X_, Y_, dcf=dcf, envelope_exponent=FIX["env"]); Yn = nz.normalize(X_, Y_)
...
def extract_coeffs_g(model, nz, dev, chunk=8192):
    ...
    d = data(); b1 = d["b1"]; RO = b1.shape[0]; C = b1.shape[-1]
    coords = torch.from_numpy(A.cartesian_grid(RO)).to(dev); P_ = coords.shape[0]
    rr = np.sqrt(A.cartesian_grid(RO)[:, 0]**2 + A.cartesian_grid(RO)[:, 1]**2).reshape(RO, RO)
    Rr = model.amplitudes(coords[:2], torch.zeros(2, dtype=torch.long, device=dev)).shape[1]
    cart = np.zeros((RO, RO, Rr, C), np.complex64)
    for c in range(C):
    ...
    cart[rr > 1.0] = 0
    a_rc = crop_img(ifft2c_mri(cart.reshape(RO, RO, Rr*C)).reshape(RO, RO, Rr, C), RO, RO)
...
def extract_pathC(model, nz, dev, chunk=8192):
    """canonical Path C coefficient maps [RO,RO,3] (complex, SENSE-combined)."""
    d = data(); b1 = d["b1"]; RO = b1.shape[0]; C = b1.shape[-1]
    coords = torch.from_numpy(A.cartesian_grid(RO)).to(dev); P_ = coords.shape[0]
    rr = np.sqrt(A.cartesian_grid(RO)[:, 0]**2 + A.cartesian_grid(RO)[:, 1]**2).reshape(RO, RO)
    cart = np.zeros((RO, RO, 3, C), np.complex64)
    for c in range(C):
        out = np.zeros((P_, 3), np.complex64)
        for i in range(0, P_, chunk):
            cc = coords[i:i+chunk]; cd = torch.full((cc.shape[0],), c, dtype=torch.long, device=dev); Amp = model.amplitudes(cc, cd)
            for r in range(3):
                pr = nz.denormalize(cc, Amp[:, r, :].contiguous()); out[i:i+chunk, r] = (pr[:, 0] + 1j * pr[:, 1]).cpu().numpy()
        cart[:, :, :, c] = out.reshape(RO, RO, 3)
    cart[rr > 1.0] = 0
    a_rc = crop_img(ifft2c_mri(cart.reshape(RO, RO, 3*C)).reshape(RO, RO, 3, C), RO, RO)
    return np.stack([np.sum(np.conj(b1)*a_rc[:, :, r, :], -1)/(np.sum(np.abs(b1)**2, -1)+1e-8) for r in range(3)], -1).astype(np.complex64)

@torch.no_grad()
def basis_at(model, t_query_s, dev):
    """F0 basis Phi(t) [len(t),3] at physical query times."""
    Tt = float(data()["times"].max()); tn = torch.tensor([2*t/Tt-1 for t in t_query_s], dtype=torch.float32, device=dev)
    return model.basis(tn).cpu().numpy()[:, :, 0].astype(np.float32)   # real channel -> [len(t),3]
...
def reconstruct(model, nz, t_query_s, dev, chunk=8192):
    ...
    d = data(); b1 = d["b1"]; RO = b1.shape[0]; C = b1.shape[-1]; Tt = float(d["times"].max())
    coords = torch.from_numpy(A.cartesian_grid(RO)).to(dev); P = coords.shape[0]
    rr = np.sqrt(A.cartesian_grid(RO)[:, 0]**2 + A.cartesian_grid(RO)[:, 1]**2).reshape(RO, RO)
    out = np.zeros((RO, RO, len(t_query_s)), np.complex64)
    for j, ts in enumerate(t_query_s):
        tn = float(2 * ts / Tt - 1); ci = np.zeros((RO, RO, C), np.complex64)
        for c in range(C):
            ...
            ci[:, :, c] = v.reshape(RO, RO)
        ci[rr > 1.0] = 0; im = ifft2c_mri(ci); out[:, :, j] = np.sum(np.conj(b1) * im, -1) / (np.sum(np.abs(b1)**2, -1) + 1e-8)
    return out

...
try:
    ...
    cs = None
    for p in [f"{GP}/results_spoke_cs/cs_slice21_f100.npy", f"{GP}/results_ref/slice_21.npz"]:
        if os.path.exists(p):
    ...
    for nm, vol, tg in methods: ax.plot(tg, np.median(vol[aorta],0), lw=1.3, label=nm)
    ax.set_xlabel("time (s)"); ax.set_ylabel("aorta ROI median signal"); ax.set_title("real in-vivo aorta curve (slice 21, AIF-gated ROI, median)"); ax.legend(); fig.tight_layout(); fig.savefig(f"{OUT}/figures/fig5_realdata_aorta_curve.png", dpi=120); plt.close(fig)

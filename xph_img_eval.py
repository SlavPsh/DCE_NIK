...
def main():
    ...
    ap.add_argument("--env", type=float, default=None); ap.add_argument("--save-prefix", default="img_eval")   # D4 envelope sweep (envsweep -> out of main aggregate)
    a = ap.parse_args(); dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    _, _, _, _, nz, dims = P.build_train(dev, env=a.env); C = dims[3]
    ...
    rot = lambda im: np.roll(im[::-1, ::-1], (1, 1), axis=(0, 1))   # centered 180deg rotation about N/2 (even-grid); im[::-1,::-1] rotates about (N-1)/2 -> a 1px shift vs truth
    def full(i):
    ...
    np.savez(f"{P.OUT}/arrays/{a.save_prefix}_{a.tag}.npz", tag=a.tag, model=a.model, rank=a.rank, width=a.width, steps=steps,
    ...
    print(f"DONE {a.tag}: best_step {steps[best]} val {val[best]:.3e} test {eb['test'][0]:.3e} imgNRMSE {eb['per'].mean():.4f} aortaCurve {eb['cur']['aorta']:.4f} | pruned ckpts -> {sorted(keep)}", flush=True)

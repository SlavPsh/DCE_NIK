...
def load_slice(zi):
    ...
    lz = r["labelGT"].shape[0] - 1 - zi                                    # labelGT z-order is REVERSED vs GroundTruth/Recon (verified)
    lab = _embed(np.array(r["labelGT"])[lz].astype(int), RO)               # (RO,RO)
...
AORTA_L, LIVER_L, SPLEEN_L = 36, 13, 30  # XCAT: cortex=23|25 (R|L), medulla=24|26 (R|L)
from scipy import ndimage as _ndi
def _central_component(mask, body, min_size=8):
    """central connected component of mask, nearest body centroid; drops peripheral arteries (label 36 tags all vessels)."""
    cc, n = _ndi.label(mask)
    if n <= 1: return mask
    cy, cx = _ndi.center_of_mass(body); best, bd = None, 1e18
    for i in range(1, n + 1):
        m = cc == i
        if m.sum() < min_size: continue
        yy, xx = _ndi.center_of_mass(m); d = (yy - cy) ** 2 + (xx - cx) ** 2
        if d < bd: bd, best = d, m
...
def rois(zi, labels=None):
    ...
    body = labels > 0; aorta = _central_component(labels == AORTA, body)   # central descending aorta only
    cortex = (labels == 23) | (labels == 25); medulla = (labels == 24) | (labels == 26)   # R|L kidney
    return dict(aorta=aorta, cortex=cortex, medulla=medulla, liver=(labels == 13), spleen=(labels == 30),
                organ=(body & ~aorta), body=body, cortex_lab='23|25', medulla_lab='24|26')

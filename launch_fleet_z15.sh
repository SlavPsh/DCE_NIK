...
S=sbatch
# --- GRASP-Pro (K=5) ---
$S --gres=gpu:a100:1 sbatch_xph_grasp.sh
...
for sd in 0 1; do
  $S --gres=gpu:4g.40gb:1 --export=ALL,MODEL=wire_ff_subspace,RANK=16,WS=1,W=768,SEED=$sd --job-name=xph-sub16s$sd sbatch_xph_img.sh
done
# --- NIK-free fully-continuous upper bound, 1 seed ---
$S --gres=gpu:a100:1 --export=ALL,MODEL=wire_ff,RANK=0,WS=0,W=768,SEED=0 --job-name=xph-free sbatch_xph_img.sh

...
def make_ring_figures(
    ...
    *,
    ...
    for ro_idx in ro_list:
        ...
        plt.legend()
        plt.tight_layout()
        figures.append(fig_mag)

    return figures


@torch.no_grad()
def make_error_map_figure(
    model,
    *,
    x_sub, y_sub,
    y_scale=1.0,
    ...
    scatter(ax[0], dRe, f"{title_prefix} log1p(|dRe|)")
    scatter(ax[1], dIm, f"{title_prefix} log1p(|dIm|)")
    scatter(ax[2], dMag, f"{title_prefix} log1p(|d|y||)")
    plt.tight_layout()
    return fig

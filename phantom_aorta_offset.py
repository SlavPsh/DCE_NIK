...
import os, sys, json, numpy as np, scipy.ndimage as ndi
...
def main():
    ...
    md = ["# phantom aorta washout offset (method minus truth, late window 100 to 175 s, one global scale on the body)", "",
          ...
          "| method | aorta offset, erode 0 / 1 / 2 / 3 px | pre-contrast offset (erode 0) | ring offset late / pre |", "|---|---|---|---|"]
    for nm, r in rows:
        er = " / ".join(f"{r[f'erode{k}_late']:+.4f} ({r[f'erode{k}_px']})" for k in range(4) if f"erode{k}_late" in r)
        md.append(f"| {nm} | {er} | {r['erode0_pre']:+.4f} | {r['ring_late']:+.4f} / {r['ring_pre']:+.4f} |")
    ...
    open(f"{P.OUT}/aorta_offset.md", "w").write("\n".join(md)); json.dump(out, open(f"{P.OUT}/aorta_offset.json", "w"), indent=1); print("\n".join(md))
    ...
    fig.suptitle("phantom aorta: late-frame error map and roi curve, full roi vs eroded roi", fontsize=11); fig.tight_layout(); fig.savefig(f"{P.OUT}/figures/aorta_offset.png", dpi=130, facecolor="white"); print("AORTA_DONE")

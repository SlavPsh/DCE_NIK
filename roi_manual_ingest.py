"""manual rois from painted pngs (rois_manual/<ds>_sl<Z>.png, any size = template size): pure red -> liver, green -> spleen, blue -> aorta (tolerant to
...
for Z in [int(s) for s in a.slices.split(",")]:
    p = f"{B}/rois_manual/{dsp.DS}_sl{Z}.png"
    ...
    np.savez(dsp.ROIS(Z), **rois, source=f"manual painting rois_manual/{dsp.DS}_sl{Z}.png, eroded {a.erode} px", aorta_tag=np.array("manual")); print("slice", Z, {k: int(v.sum()) for k, v in rois.items()})

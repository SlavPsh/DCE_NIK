...
def fwhm(c,t):
    ...
    return t[r]-t[l]
print("ktv21 (l2,1 temporal prior on k-space output), full-rank wire_ff_res, 1368 spokes + heldout. NIK-vs-NIK, so the mf ruler bias cancels.")
print(f"{'run':22} " + " ".join(f"{r+'C':>9}" for r in ROIS) + f" {'pk/ref':>7} {'FWHM s':>7}")
for w in ("0.0","0.01","0.1","1.0"):
    p=f"{B}/results_ktv21_w{w}/nik_slice_21.npy"

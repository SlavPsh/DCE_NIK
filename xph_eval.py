...
def main():
    ...
    if len(parts) > 3 and parts[3].startswith("tofts"):
        ck0 = torch.load(sorted(glob.glob(f"{P.OUT}/checkpoints/{a.tag}/ck_*.pt"))[0], map_location="cpu", weights_only=False)
        bf = ck0.get("basis_file"); bf = bf if (bf and os.path.exists(bf)) else None   # ckpt path may be from another host
        model = P.make_model_tofts(width, ks, seed, C, dev, bf); print(f"  tofts model rank {model.rank} basis {ck0.get('basis_file')}")
    else: model = P.make_model(width, ks, seed, C, dev)
    model.eval()
    ...
    rot = lambda im: np.roll(im[::-1, ::-1], (1, 1), axis=(0, 1))   # centered 180deg rotation about N/2 (even-grid); see PITFALLS 11
    def full_eval(i):
        ...
        import recon_asserts as RA; RA.check_recon(rec, Tr, mask=body, name=a.tag)          # guard: geometry/orientation/finite (raises)
    ...
    keep = {int(steps[best]), int(steps[final])}                                          # disk policy: keep only best + last checkpoint
    for c in glob.glob(f"{rundir}/ck_*.pt"):
        if int(c.split("ck_")[-1].split(".")[0]) not in keep: os.remove(c)
    print(f"DONE {a.tag}: best_step {steps[best]} val {val[best]:.3e} test {eb['test']['nmse']:.3e} imgNRMSE {eb['per_frame'].mean():.3f} aortaCurve {eb['cur_nrmse']['aorta']:.3f} | pruned ckpts -> {sorted(keep)}", flush=True)

...
import xph_grasp_lib as GL, xph_pipeline as P, xph_common as X
d = P.data(); tq = d["times"]; body = d["labels"] > 0; Tr = X.truth_at(P.ZI, tq); A = f"{P.OUT}/arrays"
dyn, Phi = GL.reconstruct(); rec0 = np.abs(dyn); tm = Tr.mean(2)
...
best = max(["id", "rot180", "fliplr", "flipud", "T"], key=lambda nm: corr(orient(rec0, nm).mean(2), tm))
...
print("edgeE(sharpness): truth %.3e | old task4-FISTA %.3e | new library GROG+NLCG %.3e" % (gradE(tm), gradE(old.mean(2)), gradE(rec.mean(2))))
print("imgNRMSE: old %.4f | new %.4f" % (nrmse(old), nrmse(rec)))
ip = np.argmin(abs(tq - 40)); vmax = np.percentile(Tr[body], 99)
fig, ax = plt.subplots(1, 3, figsize=(14, 5))
for a, im, ttl in [(ax[0], Tr[:, :, ip], "truth ~40s"), (ax[1], old[:, :, ip], "old task4-FISTA (blurry)"), (ax[2], rec[:, :, ip], "new library GROG+NLCG")]:
    a.imshow(im, cmap="gray", vmax=vmax); a.set_title(ttl); a.axis("off")
fig.tight_layout(); o = f"{P.OUT}/figures/grasp_lib_test.png"; fig.savefig(o, dpi=120); print("SAVED", o)
np.savez(f"{A}/grasp_lib_recon.npz", rec=rec.astype(np.float32), Phi=Phi, orient=best, scale=float(s))
print("GRASP_LIB_DONE")

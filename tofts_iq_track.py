...
def main():
    ...
    def iq(v, t, label):
        nt = v.shape[-1]
        if v.shape[:2] != body.shape: raise SystemExit(f"{label}: grid {v.shape[:2]} is not the dataset grid {body.shape} (wrong dataset file)")
    ...
    if dsp.DS != "p3": fin = [f for f in fin if not f[0].startswith("NIK-")]                                    # legacy p3 run paths (slice numbers overlap across datasets)

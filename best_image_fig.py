...
def main():
    ...
    enh = lambda c, t: c - np.median(c[t < 40]); mfc = np.array([mf[..., i][rois[dsp.T1]].mean() for i in range(mf.shape[-1])])
    items = []
    for spec in a.items.split(","):
        ...
        c = np.array([v[..., i][rois[dsp.T1]].mean() for i in range(v.shape[-1])]); ci = np.interp(tmf, t, c); nr = float(np.linalg.norm(enh(ci, tmf) - enh(mfc, tmf)) / np.linalg.norm(enh(mfc, tmf)))
    ...
    for j, (nm, i90, i300, hp, ae, nr) in enumerate(items):
        ...
        for k, r in ((1, dsp.T1), (1, dsp.T2)): ax[k, j].contour(rois[r][sl].astype(float), levels=[0.5], colors=["lime" if r == "cortex" else "orange"], linewidths=0.5, alpha=0.7)

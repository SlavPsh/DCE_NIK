"""k80 fair-pair panel (derived from invivo_v2_vs_nik.py). in-vivo counterpart of xph_v2_vs_nik: grasp v2 at its BEST spokes/frame (NLINE) for lam 0.25 and
...
out: figures/fig_v2_vs_nik_invivo_k80.png + v2_sweep_invivo/v2_vs_nik_invivo.json
"""
...
_, nikS = score(nik); M.append(("nik k80, 1368 sp, heldout es", nikS, ftimes(nik.shape[-1])))
for lab, f in (("grasp v2 n12 k80 lam0.25, 1368 sp", f"{GV}/gv2_slice21_n12_k80.npy"),
               ("grasp v2 n12 lam0.02, all 1704 sp, unmatched", f"{GV}/gv2_slice21_n12_lam0.02.npy")):
    v = np.abs(np.load(f)).astype(np.float32); _, vs = score(v); M.append((lab, vs, ftimes(v.shape[-1])))
tab = []
for nm, v, t in M:
...
json.dump(dict(table=tab, sweep=[{k: v for k, v in r.items() if k != "npy"} for r in rows]),
          open(f"{OUTD}/v2_vs_nik_invivo_k80.json", "w"), indent=1)
...
for pi, (plab, pt) in enumerate(PHASES):
    for mi, (nm, v, t) in enumerate(M):
        ...
        if pi == 0: ax.set_title(nm, fontsize=7)
...
fig.suptitle("real in-vivo slice 21, NO ground truth. reference = model-free nufft (31-spoke sliding window,\n"
             "~6.8 s effective temporal resolution). single setting nline 12 (2.63 s/frame) carried from the phantom. "
             "fair pair = nik k80 + grasp v2 n12 k80, both on the same 1368 spokes (v%10<8). lam 0.02 column = all spokes, unmatched, lam contrast only.", fontsize=9)
fig.savefig(f"{FIG}/fig_v2_vs_nik_invivo_k80.png", dpi=125, bbox_inches="tight")
print(f"\nSAVED {FIG}/fig_v2_vs_nik_invivo_k80.png")
print("INVIVO_K80_PANEL_DONE")

...
def main():
    ...
    ap.add_argument("--env", type=float, default=None); ap.add_argument("--runtag", default=None)          # D4 envelope sweep
    a = ap.parse_args()
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    X_, Yn, T_, C_, nz, dims = P.build_train(dev, env=a.env); C = dims[3]
    model = P.make_model_g(a.model, a.width, a.k_sigma, a.seed, C, dev, rank=a.rank, warmstart=a.warmstart); model.train()
    tag = a.runtag or f"{SHORT[a.model]}{a.rank if a.model=='wire_ff_subspace' else ''}_w{a.width}_s{a.seed}"

...
def build(mf, tmf, body, thr, erode, smooth=1.0, min_medulla_px=15, min_px=120):
    ...
    ratio = np.where(kid, first / (tail + 1e-9), 0.0)
    if smooth: ratio = np.where(kid, ndi.gaussian_filter(ratio * kid, smooth) / (ndi.gaussian_filter(kid.astype(float), smooth) + 1e-9), 0.0)   # smoothed inside the kidney only
    thr_r = np.median(ratio[kid]) if kid.any() else 0
    medulla = kid & (ratio <= thr_r); medulla = ndi.binary_opening(medulla, iterations=1)
    lab_m, nm = ndi.label(medulla); sz = ndi.sum(np.ones_like(lab_m), lab_m, range(1, nm + 1)) if nm else []
    medulla = np.isin(lab_m, [i + 1 for i in range(nm) if sz[i] >= min_medulla_px]); cortex = kid & ~medulla                       # islands dropped, cortex = the rest

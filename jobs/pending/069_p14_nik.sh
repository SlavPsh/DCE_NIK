...
RES=results/tofts_vs_patlak; OUTR=$RES/p14; SELF=$D/jobs/queue/069_p14_nik.sh; REFD=/net/beegfs/users/P101440/grasp_pro_py/results_ref_p14
...
if [ "$i" = 0 ]; then
  sbatch --dependency=afterany:$SLURM_ARRAY_JOB_ID --export=ALL,STAGE=eval --array=0 -J p14nik_eval \
    --gres=gpu:1g.12gb:1 -c 4 --mem 48G -t 6:00:00 --output=$D/jobs/log/069_p14_nik_eval_%j.out --error=$D/jobs/log/069_p14_nik_eval_%j.out $SELF && echo "eval chained afterany $SLURM_ARRAY_JOB_ID"
fi

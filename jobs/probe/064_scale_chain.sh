#!/bin/bash
# where the 1e-5 image scale comes from, stage by stage (063 died: complex64 traj gave float32 coords, finufft then demanded complex64 data)
...
kc = (tj / NGRID).astype(np.complex128); w = np.maximum(np.abs(kc), 1.0 / NX / 4.0)

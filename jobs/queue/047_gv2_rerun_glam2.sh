...
A=$(sbatch --parsable --export=ALL,STAGE=recon,PYTHONWARNINGS=ignore --array=0-53%24 -J gv2rr4 -p defq -c 8 --mem 16G -t 4:00:00 \
    --output=$D/jobs/log/047_gv2_rerun_glam2_recon_%A_%a.out --error=$D/jobs/log/047_gv2_rerun_glam2_recon_%A_%a.out $D/jobs/queue/043_gv2_rerun.sh) || exit 1
B=$(sbatch --parsable --dependency=afterok:$A --export=ALL,STAGE=post,PYTHONWARNINGS=ignore -J gv2post4 -p defq -c 4 --mem 32G -t 2:00:00 \
    --output=$D/jobs/log/047_gv2_rerun_glam2_post_%j.out --error=$D/jobs/log/047_gv2_rerun_glam2_post_%j.out $D/jobs/queue/043_gv2_rerun.sh) || exit 1

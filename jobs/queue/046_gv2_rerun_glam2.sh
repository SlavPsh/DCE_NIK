...
A=$(sbatch --parsable --export=ALL,STAGE=recon,GLAMS=0.02,0.08,0.25,X0REF=7.58e-5,PYTHONWARNINGS=ignore --array=0-53%24 -J gv2rr4 -p defq -c 8 --mem 16G -t 4:00:00 \
...
B=$(sbatch --parsable --dependency=afterok:$A --export=ALL,STAGE=post,GLAMS=0.02,0.08,0.25,X0REF=7.58e-5,PYTHONWARNINGS=ignore -J gv2post4 -p defq -c 4 --mem 32G -t 2:00:00 \

...
for r, (G, spf, msg) in enumerate(rows):
    ...
    ax.set_title(f"{spf} spokes / frame  ({m['dt_s']:.1f} s)", fontsize=14, fontweight="bold", color=OR, loc="left")
    ax.text(0.02, 0.03, f"HaarPSI {m['haarpsi']:.2f}", transform=ax.transAxes, color="w", fontsize=12)
    ax = axs[r, 1]; ax.plot(tq, tru, "k", lw=2.5, label="truth"); ax.plot(tw, c, "-o", color=OR, lw=2.2, ms=(5 if T < 50 else 2.5), label="GRASP"); ax.set_xlim(0, 120)
    ax.set_xlabel("time (s)", fontsize=12); ax.set_ylabel("aorta enhancement", fontsize=12); ax.grid(alpha=.3)
...
fig.suptitle("GRASP: spatial vs temporal resolution", fontsize=16, fontweight="bold")
